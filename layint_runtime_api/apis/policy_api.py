...
class PolicyApi(object):
    ...
    def seccomp(self, policy_id, **kwargs):
        """
        ...
        >>>     pprint(response)
        >>>
        >>> thread = api.seccomp(policy_id, callback=callback_function)
        ...
        kwargs['_return_http_data_only'] = True
        if kwargs.get('callback'):
            return self.seccomp_with_http_info(policy_id, **kwargs)
        else:
            (data) = self.seccomp_with_http_info(policy_id, **kwargs)
            return data

    def seccomp_with_http_info(self, policy_id, **kwargs):
        """
        Get a Seccomp policy derivied from a LI policy
        This method makes a synchronous HTTP request by default. To make an
        asynchronous HTTP request, please define a `callback` function
        ...
        >>>     pprint(response)
        >>>
        >>> thread = api.seccomp_with_http_info(policy_id, callback=callback_function)
        ...
        # verify the required parameter 'policy_id' is set
        if ('policy_id' not in params) or (params['policy_id'] is None):
            raise ValueError("Missing the required parameter `policy_id` when calling `seccomp`")
